...
class eppDomain {
	...
	// additional
	const STATUS_SERVER_CHANGE_DETAILS_PROHIBITED = 'serverChangeDetailsProhibited';
	const STATUS_SERVER_TRADE_PROHIBITED = 'serverTradeProhibited';
	const STATUS_SERVER_REGISTRANT_NAME_CHANGE_PROHIBITED = 'serverRegistrantNameChangeProhibited';
	const STATUS_SERVER_CONTACT_DATA_CHANGE_PROHIBITED = 'serverContactDataChangeProhibited';
	const STATUS_SERVER_DNS_CHANGE_PROHIBITED = 'serverDnsChangeProhibited';
	
    #
    # This status value can be set
    #
    ...
    public function __construct($domainname, $registrant = null, $contacts = null, $hosts = null, $period = 0, $authorisationCode = null) {
        ...
        if ($registrant instanceof eppContact) {
            throw new eppException('Registrant must be EppRegistrar\EPP\eppContactHandle or string on eppDomain creation');
        }
    ...
    public function setPeriodUnit($periodunit) {
        if (($periodunit == eppDomain::DOMAIN_PERIOD_UNIT_Y) || ($periodunit == eppDomain::DOMAIN_PERIOD_UNIT_M)) {
            ...
            throw new eppException("Domain period unit " . $periodunit . " is invalid, only y or m allowed");
        }
    }
