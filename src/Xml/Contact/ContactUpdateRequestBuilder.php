...
final class ContactUpdateRequestBuilder
{
    public function build(ContactUpdateRequest $request, string $clTrid): string
    {
        $xml = '<update>'
            . '<contact:update xmlns:contact="' . NamespaceRegistry::CONTACT . '">'
            . XmlComposer::element('contact:id', $request->id)
            . $this->statusSectionXml('contact:add', $request->addStatuses)
            . $this->statusSectionXml('contact:rem', $request->removeStatuses)
     ...
     */
    private function statusSectionXml(string $sectionName, array $statuses): string
    {
        if ([] === $statuses) {
            return '';
        }

        $statusesXml = \implode('', \array_map(
            static fn(string $status): string => '<contact:status s="' . XmlComposer::escape($status) . '"/>',
            $statuses,
        ));

        return '<' . $sectionName . '>' . $statusesXml . '</' . $sectionName . '>';
    }

    private function changeXml(ContactUpdateRequest $request): string
    {
        $hasNoChangePayload = null === $request->postalInfo
            && null === $request->voice
            && null === $request->fax
            && null === $request->email
            && null === $request->authInfo
            && null === $request->disclose;

        if ($hasNoChangePayload) {
            return '';
        }

        return '<contact:chg>'
            . (null !== $request->postalInfo ? $this->postalInfoXml($request->postalInfo) : '')
